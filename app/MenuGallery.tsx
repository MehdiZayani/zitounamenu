      ...
      >
        ...
        <div className="text-center mb-8 mt-6">
          <h1
            ...
            style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.15em", fontWeight: 400, marginBottom: 0 }}
          >
            PÂTES
          </h1>
          <span
...
const GaufresMenu = ({ section }: { section: MenuSection }) => {
  const items = section.leftItems || [];
  const gaufresSaleesItems = items.filter(item => item.description === "gaufres-salees" || item.extra === "gaufres-salees" || item.extra?.includes("salee"));
  const gaufresSucresItems = items.filter(item => item.description !== "gaufres-salees" && item.extra !== "gaufres-salees" && !item.extra?.includes("salee"));
...
const CrepesMenu = ({ section }: { section: MenuSection }) => {
  const items = section.leftItems || [];
  const crepesSaleesItems = items.filter(item => item.description === "crepes-salees" || item.extra === "crepes-salees" || item.extra?.includes("salee"));
  const crepesSucresItems = items.filter(item => item.description !== "crepes-salees" && item.extra !== "crepes-salees" && !item.extra?.includes("salee"));
...
const BriocheMenu = ({ section }: { section: MenuSection }) => {
  const items = section.leftItems || [];
  const briocheSaleesItems = items.filter(item => item.description === "brioche-salee" || item.extra === "brioche-salee" || item.extra?.includes("salee"));
  const briocheSucreesItems = items.filter(item => item.description !== "brioche-salee" && item.extra !== "brioche-salee" && !item.extra?.includes("salee"));
...
const CroissantsMenu = ({ section }: { section: MenuSection }) => {
  const items = section.leftItems || [];
  const croissantsSalesItems = items.filter(item => item.description === "croissants-sales" || item.extra === "croissants-sales" || item.extra?.includes("sauce fromage") || item.extra?.includes("sale"));
  const croissantsSucresItems = items.filter(item => !croissantsSalesItems.includes(item));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Note */}
        <p className="text-center text-sm mb-6" style={{ color: "#eedab7", fontFamily: "'Myriad Pro', sans-serif" }}>
        ...
        <div className="mt-12">
          ...
          <div className="mt-2 mb-4" style={{ borderTop: "1px solid rgba(238, 218, 183, 0.4)", width: "200px" }} />

          <div className="flex flex-col gap-1">
            {supplements.map((supp, idx) => (
              <div key={idx} className="flex justify-between gap-8">
                <span className="text-sm" style={{ color: "#eedab7", fontFamily: "'LaLuxes', serif" }}>
                  {supp.left}
                </span>
                <span className="text-sm" style={{ color: "#eedab7", fontFamily: "'LaLuxes', serif" }}>
                  {supp.right}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Note */}
        <p className="text-center text-sm mb-4" style={{ color: "#eedab7", fontFamily: "'Myriad Pro', sans-serif" }}>
...
const PetitDejMenu = ({ section }: { section: MenuSection }) => {
  ...
  const classiques = items.filter(item => item.extra !== "oeuf-brouille" && item.extra !== "benedicte");
...
const LasagnesRisottoMenu = ({ section }: { section: MenuSection }) => {
  const allItems = section.leftItems || [];
  const lasagnesItems = allItems.filter(item => item.description === "lasagnes");
  const risottoItems = allItems.filter(item => item.description !== "lasagnes");
...
const PizzasMenu = ({ section }: { section: MenuSection }) => {
  ...
  const gourmet = items.filter(item => item.extra !== "classiques");
...
const BoissonsMenu = ({ section }: { section: MenuSection }) => {
  ...
  const knownGroups = new Set([
    "cafes", "latte", "glaces", "the", "chocolat", "boissons-froides",
    "detox", "frappucino", "smoothies", "mojito", "granite", "icetea",
    "fresh-juice", "matcha"
  ]);

  // Filter items by category
  const matchCategory = (item: MenuItem, cat: string) =>
    item.description === cat || item.extra === cat;

  const cafesItems = items.filter(item => matchCategory(item, "cafes"));
  const cafesWithNespresso = cafesItems.filter(item => item.extra && item.extra !== "cafes");
  const cafesWithoutNespresso = cafesItems.filter(item => !item.extra || item.extra === "cafes");
  const latteItems = items.filter(item => matchCategory(item, "latte"));
  const glacesItems = items.filter(item => matchCategory(item, "glaces"));
  const theItems = items.filter(item => matchCategory(item, "the"));
  const chocolatItems = items.filter(item => matchCategory(item, "chocolat"));
  const boissonsItems = items.filter(item => matchCategory(item, "boissons-froides"));
  const detoxItems = items.filter(item => matchCategory(item, "detox"));
  const frappucinoItems = items.filter(item => matchCategory(item, "frappucino"));
  const smoothiesItems = items.filter(item => matchCategory(item, "smoothies"));
  const mojitoItems = items.filter(item => matchCategory(item, "mojito"));
  const graniteItems = items.filter(item => matchCategory(item, "granite"));
  const iceteaItems = items.filter(item => matchCategory(item, "icetea"));
  const freshJuiceItems = items.filter(item => matchCategory(item, "fresh-juice"));
  const matchaItems = items.filter(item => matchCategory(item, "matcha"));

  const autresBoissons = items.filter(
    item =>
      !knownGroups.has(item.description) &&
      (!item.extra || !knownGroups.has(item.extra))
  );

  const renderItems = (itemsList: MenuItem[], showDescription = false) => (
    <div className="flex flex-col gap-4">
      {itemsList.map((item, idx) => {
        const isLeft = idx % 2 === 0;
        const descText = !knownGroups.has(item.description) && item.description
          ? item.description
          : item.extra && !knownGroups.has(item.extra)
          ? item.extra
          : "";
        return (
          ...
          >
            ...
            <div className="mt-2" style={{ borderTop: "1px solid rgba(238, 218, 183, 0.4)" }} />
            {descText && (
              <p className="text-sm mt-1 px-2" style={{ color: "#eedab7", fontFamily: "'Myriad Pro', sans-serif" }}>
                {descText}
              </p>
            )}
          </div>
  ...
  const renderSectionTitle = (title: string, subtitle: string) => (
    <div className="text-center mb-5">
      <h2
        ...
        style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.12em", fontWeight: 400 }}
      >
        {title}
      </h2>
      <span
      ...
      <div className="mx-auto mt-2" style={{ width: 40, borderTop: "1px solid rgba(238, 218, 183, 0.5)" }} />
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* AUTRES BOISSONS / SÉLECTION */}
        {autresBoissons.length > 0 && (
          <div className="mb-8">
            {renderSectionTitle("SÉLECTION", "Boissons")}
            {renderItems(autresBoissons, true)}
          </div>
        )}
...
const ChichaMenu = ({ section }: { section: MenuSection }) => {
  const items = section.leftItems || [];
  const classiqueItems = items.filter(item => item.description === "classique" || item.extra === "classique");
  const adalyaItems = items.filter(item => item.description !== "classique" && item.extra !== "classique");
      ...
      >
        ...
        <div className="text-center mb-12 mt-8">
          <h1
            ...
            style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.25em", fontWeight: 400, marginBottom: 0 }}
          >
            CHICHA
          </h1>
          <span
          ...
          <div className="mx-auto mt-4" style={{ width: 120, borderTop: "1px solid rgba(238, 218, 183, 0.5)" }} />
        </div>
            ...
            <div key={idx} className="mb-6">
              <div className="flex justify-between items-center px-4">
                <h3
                  ...
                  style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.15em" }}
                >
                  {item.title}
                </h3>
                <span
              ...
              <div className="mt-2 mx-4" style={{ borderTop: "1px solid rgba(238, 218, 183, 0.4)" }} />
            </div>
          ))}
        </div>
...
const TwoColumnMenu = ({ section, showBackground = true }: { section: MenuSection; showBackground?: boolean }) => {
  ...
  const frame = (
    ...
    >
      <div className="flex-1 border border-[#eedab7] p-8 relative">
        {/* Title */}
        <div className="text-center relative">
          <h1
            className="mb-0 text-4xl sm:text-5xl md:text-[96px]"
            style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.18em", fontWeight: 300, marginBottom: 4 }}
          >
            {section.label}
          </h1>
          ...
          >
            <span className="text-lg sm:text-2xl md:text-[52px]" style={{ fontFamily: section.subtitle === "Menu" ? "'zoxi-regular', serif" : "'Tangerine', cursive", letterSpacing: '0.02em' }}>{section.subtitle}</span>
          </span>

          <div className="mx-auto my-5" style={{ width: 56, borderTop: "2px solid rgba(238, 218, 183, 0.9)" }} />
          <p className="text-xs sm:text-sm" style={{ color: "#eedab7", marginTop: 8, fontFamily: "'Myriad Pro', sans-serif" }}>
              ...
              <div key={idx} className="flex flex-col gap-4">
                {row.left && (
                  <div className="w-full sm:w-[60%] p-4" style={{ background: "transparent" }}>
                    <div className="flex justify-between items-baseline">
                      <h3 className="text-xl sm:text-3xl md:text-[48px]" style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.12em", lineHeight: 1 }}>{row.left.title}</h3>
                      <div className="text-lg sm:text-2xl md:text-[36px]" style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7" }}>{row.left.price}</div>
                    </div>
                    <div className="mt-3" style={{ borderTop: "2px solid rgba(239,232,214,0.95)", width: "100%" }} />
                    {row.left.description && <p className="text-sm sm:text-base" style={{ color: "#eedab7", marginTop: 12, fontFamily: "'Myriad Pro', sans-serif" }}>{row.left.description}</p>}
                  </div>
                )}
                ...
                {row.right && (
                  <div className="w-full sm:w-[60%] p-4 sm:ml-auto" style={{ background: "transparent" }}>
                    <div className="flex justify-between items-baseline">
                      <h3 className="text-xl sm:text-3xl md:text-[48px]" style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7", letterSpacing: "0.12em", lineHeight: 1 }}>{row.right.title}</h3>
                      <div className="text-lg sm:text-2xl md:text-[36px]" style={{ fontFamily: "'LaLuxes', serif", color: "#eedab7" }}>{row.right.price}</div>
                    </div>
                    <div className="mt-3" style={{ borderTop: "2px solid rgba(239,232,214,0.95)", width: "100%" }} />
                    {row.right.description && <p className="text-sm sm:text-base" style={{ color: "#eedab7", marginTop: 12, textAlign: "left", fontFamily: "'Myriad Pro', sans-serif" }}>{row.right.description}</p>}
                  </div>
                )}
              </div>
            ))}
...
export default function MenuGallery({ initialData }: { initialData?: MenuSection[] }) {
  // Use live database menu passed from server (zero flicker)
  const [source, setSource] = useState<MenuSection[]>(initialData && initialData.length > 0 ? initialData : menuData);
      ...
      <main>
        ...
        <header id="top" className="w-full flex items-center justify-center" style={{ backgroundImage: "url('/coffee/boissons-bg.png')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
          <div className="w-[92vw] sm:w-[88vw] max-w-[1200px] text-center py-16 sm:py-28">
            <h2 style={{ fontFamily: "'zoxi-regular', serif", color: '#eedab7', fontSize: 'clamp(48px, 8vw, 160px)', letterSpacing: '0.22em', margin: 0 }}>Menu</h2>
            <p style={{ fontFamily: "'Myriad Pro', sans-serif", color: '#c9782b', fontSize: 'clamp(16px, 2.2vw, 28px)', marginTop: 8 }}>Savourez chaque instant</p>
            <div style={{ width: 56, margin: '18px auto 0', borderTop: '2px solid rgba(238, 218, 183, 0.9)' }} />
          </div>
        </header>
